...
def _print_node(msg: dict) -> None:
    ...
    console.print(f"  [bold green][+] NODE_DISCOVERED (d={depth} s={score:.1f})[/bold green] {url}{param_str}")

def _print_summary(parser: ResultParser) -> None:
    stats = parser.stats
    table = Table(title="SYSTEM SCAN RECAP", show_header=True, header_style="bold magenta")
...
@click.option("--header", "-H", default=None, help="Custom HTTP header (e.g., Cookie: session=abc)")
@click.option("--rps", default=None, type=float, help="Requests per second (overrides config).")
@click.option("--rpm", default=None, type=float, help="Requests per minute (overrides rps).")
@click.option("--timeout", default=None, type=int, help="Hard timeout in seconds.")
@click.option("--severity", default=None, help="Comma-separated severity filter, e.g. critical,high")
@click.option("--output", "-o", default=None, help="Output directory for reports.")
@click.option("--format", "fmt", default="json", type=click.Choice(["json", "csv", "pdf"]), show_default=True)
@click.option("--profile", default=None, help="Scan profile from settings.yaml (e.g. full, fast, stealth)")
@click.option("--show-nodes", is_flag=True, default=False, help="Print discovered endpoint nodes live.")
@click.option("--dry-run", is_flag=True, default=False, help="Print plan without running the scan.")
...
def cmd_scan(
    ...
    if profile:
        if "profiles" in cfg and profile in cfg["profiles"]:
            ...
            console.print(f"  [bold red][ERROR] Profile '{profile}' not found in {SETTINGS_FILE}[/bold red]")
            sys.exit(1)
    ...
    console.print(Panel(f"[bold cyan]Target:[/bold cyan] {target}\n[bold cyan]Speed:[/bold cyan] {final_rps:.2f} RPS ({effective_rpm:.2f} RPM)\n[bold cyan]Severities:[/bold cyan] {', '.join(sev_list)}", title="Blue Whale Initialization", expand=False))
    
    if dry_run:
        console.print("  [bold yellow][DRY RUN] Engine will not be spawned.[/bold yellow]\n")
            ...
            try:
                async for msg in executor.run():
                    ...
                    if msg_type == "vulnerability":
                        ...
                        console.print(f"  [bold red][OAST HIT][/bold red] {protocol} interaction from {remote_addr} (ID: {identifier})")
                        synthetic = {
                        ...
                        if len(detail) > 40:
                            detail = detail[:37] + "..."
                        progress.update(task_id, completed=prog_val, description=f"[bold green][{phase.upper()}][/bold green] {detail}")

                    elif msg_type == "scan_done":
                        progress.update(task_id, completed=100, description="[bold cyan]Scan Complete[/bold cyan]")
                        ...
                        console.print(f"  [yellow]DRY RUN: would scan {msg.get('target')} (job {msg.get('job_id')})[/yellow]")
...
@click.option("--format", "fmt", default="pdf", type=click.Choice(["pdf"]), show_default=True)
...
def cmd_report(
    ...
    console.print(f"  [cyan]Generating {fmt.upper()} report from {results_file}...[/cyan]")
...
@click.option("--force", is_flag=True, default=False, help="Rebuild engine even if binary exists.")
...
@cli.command("paths")
def cmd_paths() -> None:
    console.print(Panel("[bold cyan]Blue Whale Path Resolution[/bold cyan]", expand=False))
    ...
    for name, p in all_paths().items():
        status = "[bold green][OK][/bold green]" if p.exists() else "[bold red][XX][/bold red]"
